...
async function SearchPage({ searchParams }: IProps) {
    ...
    const resultSearch = await searchProduct({ query, sort_by, sort_dir, page_number, page_size: pageSize });
                ...
                <div className='flex items-center gap-x-4'>
                    ...
                    <div className='space-x-1'>
                        <Link
                            ...
                            href={{
                                pathname: '/search',
                                query: { query, sort_by, sort_dir, page_number: (Number(page_number) ?? 1) - 1 },
                            }}
                            className={cn(buttonVariants({ variant: 'secondary', size: 'icon' }), {
                                'pointer-events-none opacity-50': page_number === 1,
                            })}
                        >
                            <ChevronLeftIcon />
                        </Link>
                        <Link
                            ...
                            className={cn(buttonVariants({ variant: 'secondary', size: 'icon' }), {
                                'pointer-events-none opacity-50': page_number === resultSearch.totalPage,
                            })}
                        >
                            <ChevronRightIcon />
                        </Link>
